...
def _layer_check(net):
    ...
    for layer in net:
        ...
        is_normizh = isinstance(layer, snn.NormIzh)
...
def _layer_reset():
    ...
    if is_normizh:
        snn.NormIzh.reset_hidden()  # reset hidden state according to Izhikevich
        snn.NormIzh.detach_hidden()
...
def _final_layer_check(net):
    ...
    if isinstance(list(net._modules.values())[-1], snn.Alpha):
        return 4
    if isinstance(list(net._modules.values())[-1], snn.NormIzh):
        return 3
